"""Structural conformance tests for the whatsapp/ plugin bundle.
...
PLUGIN_ROOT = Path(__file__).resolve().parents[2] / "whatsapp"
...
class TestPortableManifest:
    ...
    def test_points_at_wrapper_repo(self, portable_manifest: dict) -> None:
        """The portable manifest must reference this wrapper repository, not
        the vendored upstream lharries/whatsapp-mcp project."""
        for field in ("homepage", "repository"):
            assert "bchewy" in portable_manifest[field], field
            assert "lharries" not in portable_manifest[field], field
...
class TestPortableMcp:
    ...
    def test_declares_whatsapp_stdio_server(self, portable_mcp: dict) -> None:
        server = portable_mcp["mcpServers"]["whatsapp"]
    ...
    def test_launch_script_is_bundled(self, portable_mcp: dict) -> None:
        server = portable_mcp["mcpServers"]["whatsapp"]
        expanded = [
            arg.replace("${PLUGIN_ROOT}", str(PLUGIN_ROOT)) for arg in server["args"]
        ]
        script_args = [arg for arg in expanded if arg.endswith(".sh")]
        assert script_args, "expected an arg pointing at the launch script"
        for arg in script_args:
            resolved = Path(arg).resolve()
            assert resolved.is_file(), f"launch script missing: {resolved}"
...
class TestDualLayoutSync:
    ...
    def test_same_server_launch_in_both_configs(
        ...
        portable = portable_mcp["mcpServers"]["whatsapp"]
        codex = codex_mcp["mcpServers"]["whatsapp"]
