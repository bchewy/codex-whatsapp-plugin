...
def _err(msg: str) -> None:
    """Write diagnostics to stderr so they don't corrupt stdio MCP framing."""
    print(msg, file=sys.stderr, flush=True)


_legacy_store = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), '..', 'whatsapp-bridge', 'store'
)
_store_dir = os.environ.get("WHATSAPP_MCP_STORE_DIR", _legacy_store)
MESSAGES_DB_PATH = os.path.join(_store_dir, "messages.db")

_bridge_port = os.environ.get("WHATSAPP_BRIDGE_PORT", "8080")
WHATSAPP_API_BASE_URL = f"http://127.0.0.1:{_bridge_port}/api"
WHATSAPP_BRIDGE_TOKEN = os.environ.get("WHATSAPP_BRIDGE_TOKEN", "").strip()

def _bridge_headers() -> dict:
    if not WHATSAPP_BRIDGE_TOKEN:
        return {}
    return {"X-WhatsApp-Bridge-Token": WHATSAPP_BRIDGE_TOKEN}

def _require_bridge_token() -> Optional[str]:
    if WHATSAPP_BRIDGE_TOKEN:
        return None
    return "WHATSAPP_BRIDGE_TOKEN is not configured; start the server through whatsapp/scripts/run-mcp.sh"

def _bridge_ready() -> Tuple[bool, str]:
    token_error = _require_bridge_token()
    if token_error:
    ...
    try:
        ...
        if response.status_code != 200:
            return False, f"WhatsApp bridge health check failed: HTTP {response.status_code} - {response.text}"
        result = response.json()
        if result.get("success") is True:
            return True, ""
        return False, "WhatsApp bridge health check failed"
    except requests.RequestException as e:
        return False, f"WhatsApp bridge health check failed: {str(e)}"
    except json.JSONDecodeError:
        return False, f"WhatsApp bridge health check returned invalid JSON: {response.text}"

def _canonical_media_path(media_path: str) -> Tuple[Optional[str], Optional[str]]:
    expanded_path = os.path.expanduser(media_path)
    if not os.path.isabs(expanded_path):
        return None, "Media path must be an absolute path to avoid sending an unintended local file"

    resolved_path = os.path.realpath(expanded_path)
    if not os.path.isfile(resolved_path):
        return None, f"Media file not found: {resolved_path}"
...
def get_sender_name(sender_jid: str) -> str:
    try:
        ...
        _err(f"Database error while getting sender name: {e}")
        return sender_jid
    finally:
        if 'conn' in locals():
            conn.close()
...
def format_message(message: Message, show_chat_info: bool = True) -> None:
    ...
    try:
        ...
        _err(f"Error formatting message: {e}")
    return output
...
def send_file(recipient: str, media_path: str) -> Tuple[bool, str]:
    try:
        ...
        media_path, path_error = _canonical_media_path(media_path)
        if path_error:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if result.get("success", False):
                ...
                _err(f"Media downloaded successfully: {path}")
                return path
            else:
                _err(f"Download failed: {result.get('message', 'Unknown error')}")
                return None
        else:
            _err(f"Error: HTTP {response.status_code} - {response.text}")
            return None
            
    except requests.RequestException as e:
        _err(f"Request error: {str(e)}")
        return None
    except json.JSONDecodeError:
        _err(f"Error parsing response: {response.text}")
        return None
    except Exception as e:
        _err(f"Unexpected error: {str(e)}")
        return None
